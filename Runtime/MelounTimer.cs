...
namespace FruitBowl {
	public class MelounTimer : MonoBehaviour {
		...
		/// <summary>
		/// Update triggered by external sources.
		/// </summary>
		public void ExternalFixedUpdate() {
			if(externalUpdate){
				...
				Debug.LogWarning("Trying to trigger external update of Meloun timer " + timerName + ", but timer has no external update enabled. External update ignored.");
			}
		}
